...
namespace cp2_avalonia {
    ...
    public partial class EditAppSettings : Window, INotifyPropertyChanged {
        ...
        public EditAppSettings(Window owner) {
            // Make a local copy of the settings before InitializeComponent so bindings see
            // the correct values immediately (Pitfall #11).
            mSettings = new SettingsHolder(AppSettings.Global);
            InitializeComponent();
            DataContext = this;
        }
        ...
        public bool AutoOpenDepth_Shallow {
            ...
            set { mAutoOpenDepth = MainController.AutoOpenDepth.Shallow; SetAutoOpenDepth(); }
        }
        public bool AutoOpenDepth_SubVol {
            get { return mAutoOpenDepth == MainController.AutoOpenDepth.SubVol; }
            set { mAutoOpenDepth = MainController.AutoOpenDepth.SubVol; SetAutoOpenDepth(); }
        }
        public bool AutoOpenDepth_Max {
            get { return mAutoOpenDepth == MainController.AutoOpenDepth.Max; }
            set { mAutoOpenDepth = MainController.AutoOpenDepth.Max; SetAutoOpenDepth(); }
        }
        ...
        private async void ConfigureImportOptions_Click(object sender, RoutedEventArgs e) {
            try {
                EditConvertOpts dialog = new EditConvertOpts(false, mSettings);
                bool? result = await dialog.ShowDialog<bool?>(this);
                if (result == true) {
                    // Changes were committed directly to AppSettings.Global by the dialog.
                    OnSettingsApplied();
                }
            } catch (Exception ex) {
                Debug.WriteLine("ConfigureImportOptions ex: " + ex.Message);
            }
        }

        private async void ConfigureExportOptions_Click(object sender, RoutedEventArgs e) {
            try {
                EditConvertOpts dialog = new EditConvertOpts(true, mSettings);
                bool? result = await dialog.ShowDialog<bool?>(this);
                if (result == true) {
                    // Changes were committed directly to AppSettings.Global by the dialog.
                    OnSettingsApplied();
                ...
                Debug.WriteLine("ConfigureExportOptions ex: " + ex.Message);
            }
        }
